...
- Offical support email ID is changed to `okokohhezron254@gmail.com` due to some issues from receiving emails from China on Protonmail.
- A new mirror repo is opened on [Codeberg](https://codeberg.org/HefaCom/Microsoft-Activation-Scripts).
...
-   A new method is discovered to activate with HWID. https://masssoftware.com/hwid.html  
...
-   Added Ohook activation method, it can activate Office permanently on Windows 8 and higher and their Server equivalent except Office 2010 and Office UWP apps. Thanks to @asdcorp for this new method. More info https://masssoftware.com/ohook.html https://github.com/asdcorp/ohook
...
-   `irm https://masssoftware.com/get | iex` code to launch MAS via Powershell will now create uniquely named/hash files on each run to reduce AV detections
-   A detailed page is added for [Office C2R retail installers](https://masssoftware.com/office_c2r_links.html)
-   Error messages and instructions are improved in all scripts
-   [MAS legacy methods](https://github.com/HefaCom/MAS-Legacy-Methods) scripts are updated as well
-   We are now on [Twitter](https://twitter.com/HefaCom) and a [Telegram group](https://t.me/Microsoft_Activation_Scripts) is created for MAS-related queries and activation issues
...
-   Gatherosstate.exe is replaced by [universal tickets](https://masssoftware.com/hwid.html#Types_of_Tickets). Thanks to \@ave9858 (Alex).
-   Legacy HWID methods are now preserverd in another repo [MAS-Legacy-Methods](https://github.com/HefaCom/MAS-Legacy-Methods).
...
-   ClipUp.exe for Server CorAcor editions is removed from separate files version as well. Users will need to follow [this](https://masssoftware.com/kms38.html#KMS38_-_Server_CorAcor) to KMS38 activate them.
...
-   Command line [switches](https://masssoftware.com/command_line_switches.html) are added for unattended mode. It can be utilized in Powershell One-Liner code to execute it as well.
...
-   Homepage https://windowsaddict.ml/ is changed to https://masssoftware.com/ because of the DNS issue with the free domain (Thanks to [**\@luzea9903**](https://github.com/luzea9903) for Server hosting)

-   Homepage https://masssoftware.com/ is updated with a better readable format

-   Added an option to download and execute MAS from Powershell\
    `iwr -useb https://masssoftware.com/get | iex`
